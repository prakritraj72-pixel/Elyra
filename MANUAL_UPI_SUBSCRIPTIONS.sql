...
-- Current pricing: 1 Month ₹199 / 1 Year ₹699.
-- Run this whole file once in Supabase SQL Editor for a fresh setup.
-- If the old version was already run, use MIGRATE_SUBSCRIPTION_PRICING_TO_199_699.sql.

create table if not exists public.subscription_payment_requests (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  plan text not null check (plan in ('monthly','yearly')),
  amount numeric(10,2) not null check (amount in (199,699)),
...
create unique index if not exists subscription_payment_requests_transaction_id_idx on public.subscription_payment_requests (lower(transaction_id));
create index if not exists subscription_payment_requests_user_id_idx on public.subscription_payment_requests (user_id);
create index if not exists subscription_payment_requests_status_idx on public.subscription_payment_requests (status, created_at desc);
...
returns table(id uuid,user_id uuid,user_email text,plan text,amount numeric,transaction_id text,status text,created_at timestamptz,reviewed_at timestamptz,rejection_reason text)
language plpgsql security definer set search_path = public, auth as $$
begin
  if auth.uid()<>'2b6e3748-1702-4cdc-b38d-b826577af65a'::uuid then raise exception 'Access denied'; end if;
  return query select r.id,r.user_id,u.email::text,r.plan,r.amount,r.transaction_id,r.status,r.created_at,r.reviewed_at,r.rejection_reason from public.subscription_payment_requests r left join auth.users u on u.id=r.user_id order by r.created_at desc;
end; $$;
...
begin
  ...
  if v_request.amount<>case when v_request.plan='monthly' then 199 else 699 end then return jsonb_build_object('success',false,'message','Payment amount does not match the current plan price.'); end if;
  if exists(select 1 from public.subscriptions s where s.user_id=v_request.user_id and s.status='active' and (s.expires_at is null or s.expires_at>now())) then return jsonb_build_object('success',false,'message','This user already has an active subscription. Reject this request instead.'); end if;
...
begin
  if auth.uid()<>'2b6e3748-1702-4cdc-b38d-b826577af65a'::uuid then raise exception 'Access denied'; end if;
  return jsonb_build_object('total',(select count(*) from public.subscription_payment_requests),'pending',(select count(*) from public.subscription_payment_requests where status='pending'),'approved',(select count(*) from public.subscription_payment_requests where status='approved'),'rejected',(select count(*) from public.subscription_payment_requests where status='rejected'),'approved_amount',coalesce((select sum(amount) from public.subscription_payment_requests where status='approved'),0));
end; $$;
