-- ELYRA SUBSCRIPTION PRICING MIGRATION
-- Current public pricing:
-- 1 Month = ₹199
-- 1 Year  = ₹699
-- Run this ONCE in Supabase SQL Editor if the original subscription SQL was already run.

begin;

-- Replace the old amount constraint (199 / 499) with the current pricing.
alter table public.subscription_payment_requests
  drop constraint if exists subscription_payment_requests_amount_check;

alter table public.subscription_payment_requests
  add constraint subscription_payment_requests_amount_check
  check (amount in (199,699));

-- Recreate the submission RPC so the server-side amount cannot be spoofed.
...
-- Recreate admin approval so monthly subscriptions expire after exactly 1 month.
...
begin
  ...
  if v_request.amount <> case when v_request.plan='monthly' then 199 else 699 end then
    return jsonb_build_object('success',false,'message','Payment amount does not match the current plan price.');
  end if;
  ...
  insert into public.subscriptions(
    ...
    v_request.user_id, v_request.plan, v_request.amount, 'active',
    v_started, v_expires, v_request.transaction_id, 'manual_upi'
